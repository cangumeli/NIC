...
function img2vec(img; crop=true, crop_target=224, rescale = false, minmin_scale=256, minmax_scale=256, flip=true, prep=false)
    if prep
    ...
    I = zeros(Float32, size(r)[2], size(r)[3], 3, 1)
    I[:,:,1,1] = r[1,:,:]
    I[:,:,2,1] = r[2,:,:]
    I[:,:,3,1] = r[3,:,:]
    return I
